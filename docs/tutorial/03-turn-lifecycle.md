...
`ApplicationSession` 持有整个会话的对话历史（`messages`）、turn 计数器（`nextTurn`）和"当前活跃 turn"三件套（`activeRunner` / `activeGraphThread` / `activeTurn`），是把 REPL 输入翻译成图执行的那一层。
...
| `createRunner(turn, graphThread, cancellationToken, renderer)` | 同名参数含义同上 | 每 turn 现场组装一台运行器，下文单讲。 |
