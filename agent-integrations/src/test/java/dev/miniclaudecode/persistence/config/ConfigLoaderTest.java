...
class ConfigLoaderTest {
  ...
  void usesDefaultsWhenUserConfigDoesNotExist() {
    ...
    Assertions.assertThat(config.execution()).isEqualTo(ExecutionConfig.defaults());
  ...
  void parsesPlanningAndSqliteMemoryConfiguration() throws Exception {
    ...
    Files.writeString(
        ...
        "planning:\n"
            ...
            + "  backend: sqlite\n"
            + "  top-k: 5\n"
            + "  max-injection-tokens: 1200\n"
            + "  consolidate-after: 12\n"
    ...
    Assertions.assertThat(config.execution()).isEqualTo(new ExecutionConfig("hybrid", 1, 2, 0));
    Assertions.assertThat(config.memory())
        .isEqualTo(new MemoryConfig(false, "sqlite", 5, 1200, 12, false, "signal-gated"));
  }
