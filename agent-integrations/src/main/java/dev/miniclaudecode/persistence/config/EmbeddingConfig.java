 ...
 *
 * <p>{@code REMOTE} calls an OpenAI-compatible {@code /v1/embeddings} endpoint and is the
 * production semantic-retrieval option. {@code FAST} is an offline hashing fallback with no local
 * model dependency. The vector {@code dimensions} must be declared before indexing because Lucene
 * fixes the vector-field dimension on first use.
 */
