...
public final class ReflexionExtractor {
  ...
  public Optional<AceBullet> extract(
    ...
    // Post-turn learning is deliberately narrower than a Reflection loop: only a concrete,
    // successful verification result is durable authority. Failures and assistant deductions stay
    // in the JSONL episode and are never promoted to long-term memory.
    if (!verifiedChange) {
    ...
    String fallback = FALLBACK_SUCCESS_LESSON;
    String lesson = this.lessonGenerator.apply(context).orElse(fallback);

    String trigger = "verified workspace change";
    String safeEvidence = "verification command succeeded";
    return Optional.of(
        new AceBullet(
            null,
            trigger,
