    ...
    private HookResult executeCommand(Hook h, HookContext ctx) {
        ...
        try {
            ...
            if (System.getProperty("os.name", "").toLowerCase(java.util.Locale.ROOT).contains("win")) {
                String windowsCommand = command.replace("$MEWCODE_FILE_PATH", "%MEWCODE_FILE_PATH%");
                if (windowsCommand.matches("sleep\\s+\\d+")) {
                    windowsCommand = "powershell.exe -NoProfile -Command Start-Sleep -Seconds "
                            + windowsCommand.replaceFirst("sleep\\s+", "");
                }
                pb = new ProcessBuilder("cmd.exe", "/d", "/s", "/c", windowsCommand);
            } else {
            ...
            env.put("MEWCODE_FILE_PATH", ctx.filePath() != null ? ctx.filePath() : "");
